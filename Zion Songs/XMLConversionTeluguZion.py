...
# Source Telugu cleaned file
src_file = Path(r"SourceFile/all_songs_telugu_cleaned.txt")
out_dir = Path("OutputXML_telugu")
...
pallavi_re = re.compile(r"^పల్లవి\s*:\s*(.*)")
anupallavi_re = re.compile(r"^అనుపల్లవి\s*:\s*(.*)")
...
def create_song_xml(song_num, en_title, te_title, pallavi_lines, anupallavi_lines, verses_lines):
    ...
    ET.SubElement(titles, "title").text = te_title
    ET.SubElement(titles, "title").text = song_num

    # Build verseOrder (3 rules)
    ...
    if not pallavi_lines and not anupallavi_lines:
        ...
        # Rule 3: Pallavi + Anupallavi + Verses + repeat Pallavi
        order_parts = ["c1", "c2"]
        for i in range(1, verses_count + 1):
            order_parts.append(f"v{i}")
    ...
    ET.SubElement(songbooks, "songbook", {"name": "Zion Songs Telugu", "entry": song_num})
    ...
    def add_lines(verse_name, lines_list):
        ...
        verse_text = "\n".join(lines_list)
        ET.SubElement(verse, "lines").text = f"{{lang-tel}}{verse_text}{{/lang-tel}}"
...
song_num, en_title, te_title = None, None, None
...
def flush_song():
    global song_num, en_title, te_title, pallavi_lines, anupallavi_lines, verses_lines, current_verse_lines, current_state
    if song_num:
        # Add last verse lines if any
        if current_state == "verse" and current_verse_lines:
            verses_lines.append(current_verse_lines)
        song_xml = create_song_xml(song_num, en_title or "", te_title or "", pallavi_lines, anupallavi_lines, verses_lines)
...
for line in lines + [""]:
    ...
    if line.startswith("Song Number:"):
        ...
        te_title = te_title_re.match(line).group(1)
